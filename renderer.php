...
class tool_mergeusers_renderer extends plugin_renderer_base
{
	...
	public function progress_bar(array $items): string
	{
		...
		return html_writer::tag('div', join(get_separator(), $items),
	...
	public function logs_page(array $logs): string
	{
		...
		if(empty($logs)) {
			...
			$table->head = [get_string('olduseridonlog', 'tool_mergeusers'), get_string('newuseridonlog', 'tool_mergeusers'), get_string('date'), get_string('status'), ''];
