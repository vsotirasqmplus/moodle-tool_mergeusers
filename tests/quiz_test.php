...
class tool_mergeusers_quiz_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        $maninstance1 = $DB->get_record(
                'enrol', ['courseid' => $this->course1->id, 'enrol' => 'manual'],
                '*', MUST_EXIST
        );

        $maninstance2 = $DB->get_record(
                'enrol', ['courseid' => $this->course2->id, 'enrol' => 'manual'],
                '*', MUST_EXIST
        );
     ...
     */
    private function add_quiz_to_course($course): testable_assign {
     ...
     * @return array
     */
    private function get_fiftypercent_answers(): array {
        return [1 => ['answer' => 'frog'], 2 => ['answer' => '3.15']];
    }
     ...
     * @return testable_assign
     */
    private function get_user_quiz_grade($user, $quiz, $course): testable_assign {
    ...
    private function get_hundredpercent_answers(): array {
        return [1 => ['answer' => 'frog'], 2 => ['answer' => '3.14']];
    }
